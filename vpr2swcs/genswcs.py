...
# run through similar switch generation as normal lut/ff
			...
			sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
		    if nsb.name in ["mite2[0]", "mult[0]","ladder_filter[0]","hhneuron[0]","hh_neuron_b_debug[0]","dendiff[0]",'speech[0]','dendrite_4x4[0]','inv[0]','bias_gen[0]']:
			sub_outputs=subblock.findall("outputs")[0].findall("port")[0].text.split()
			#pdb.set_trace()
			...
			#pdb.set_trace()
                        if from_sub_name ==['SR4[0]']:
                            ...
                            try:
                                ...
                                pdb.set_trace()
			elif from_sub_name in ['mite2[0]','mult[0]','bias_gen[0]','ladder_filter[0]','hh_neuron_b_debug[0]','dendiff[0]','hhneuron[0]','speech[0]','dendrite_4x4[0]','inv[0]']:
			    #pdb.set_trace()
                            try:
                                #if i<2:
