...
global RAMP_ADC_check sftreg_check Signal_DAC_check GPIO_IN_check MITE_ADC_check plcloc ladder_count ONchip_ADC; 
RAMP_ADC_check=0; sftreg_check=0; Signal_DAC_check=0; GPIO_IN_check=0; MITE_ADC_check=0; ladder_count=0; ONchip_ADC =0; sftreg_count=0;
...
dac_loc_idx=0; dac_buf_loc_idx=0; gpin_loc_idx=0; adc_loc_idx=0;
...
for i =1:no
    ...
    //elseif ( length(scs_m.objs(i) )==1) then //disp("deleted block",i);
        ...
        elseif (scs_m.objs(i).gui== "adc") then prime_ops(op_count)=j; op_count=op_count+1; blk_name.entries(j)=  scs_m.objs(i).gui; blk_objs(objnum)=j; objnum=objnum+1;
        ...
        elseif   (scs_m.objs(i).gui== "pad_outd") then prime_ops(op_count)=j; op_count=op_count+1; blk_name.entries(j)=  scs_m.objs(i).gui; blk_objs(objnum)=j; objnum=objnum+1;
            ...
            elseif (scs_m.objs(i).gui =='meas_volt') then mite_adc = 1; //add_gnd1=%t;
            elseif (scs_m.objs(i).gui =='TIA') then mite_adc = 1; //add_gnd1=%t;
            elseif (scs_m.objs(i).gui =='GENARB_f')  then
                ga_blk_num = ga_blk_num+1; ga_idx(ga_blk_num,1).entries(1,1)=i; ga_idx(ga_blk_num,1).entries(1,2)=size(evstr(scs_m.objs(i).model.opar(1)), "r");
            elseif (scs_m.objs(i).gui =='gpio_in' | scs_m.objs(i).gui =='gpio_in_fixloc')  then
                gpin_blk_num = gpin_blk_num+1; gpin_idx(gpin_blk_num,1).entries(1,1)=i; gpin_idx(gpin_blk_num,1).entries(1,2)=size(evstr(scs_m.objs(i).model.opar(1)), "r");
            ...
            elseif (scs_m.objs(i).gui== "dff") then fix_gnd = 1; fix_vdd = 1; add_tgates4logic = 1; number_tgates = max(3,number_tgates);
            ...
            elseif (scs_m.objs(i).gui== "mux4_1") then mux4_1 = 1; 
            elseif (scs_m.objs(i).gui== "vmmwta") then plcvpr = %t; // fix_gnd = 1;
...
if dig_blk==1 then prime_ip_string= '.inputs rst'; fd_io= mopen(fname+'.pads','wt'); mputl('rst 8 0 0  #tgate[0]',fd_io); mclose(fd_io);
...
if mux4_1==1 then prime_ip_string=strcat([prime_ip_string," sel1 sel2 sel3 sel4"]); end
...
for s=1: length(prime_ops)
    ...
    elseif blk_name(prime_ops(s)).entries == 'adc' then
        for ss=1:scs_m.objs(prime_ops(s)).model.ipar(1)
            prime_op_string= prime_op_string + ' out_adc_" + string(ss);
        end    
    else
        for ss=1:scs_m.objs(prime_ops(s)).model.ipar(1)
...
if makeblk == %f then
    fd_w= mopen (fname+'.blif','wt'); mputl(strcat([".model"," ",fname])  ,fd_w); mputl(prime_ip_string,fd_w); mputl(prime_op_string,fd_w); mputl("  ",fd_w); mclose(fd_w);
...
if(makeblk) then 
    ...
    if plcvpr then
        if addvmm then unix_s('/home/ubuntu/rasp30/vtr_release/vpr/vpr /home/ubuntu/rasp30/vpr2swcs/./arch/'+arch+'_vmm_arch.xml ' + path + fname + '  -route_chan_width 17 -timing_analysis off -fix_pins ' + path + fname + '.pads -nodisp');
        ...
        // generate switches
        if addvmm then unix_s('python /home/ubuntu/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -vmm -route' + brdtype);
        else unix_s('python /home/ubuntu/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path+ ' -route' + brdtype);
        end
    else
        if addvmm then unix_s('python /home/ubuntu/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + ' -vmm' + brdtype);
        else unix_s('python /home/ubuntu/rasp30/vpr2swcs/genswcs.py -c ' + path + fname + ' -d '+ path + brdtype);
        end
    end
