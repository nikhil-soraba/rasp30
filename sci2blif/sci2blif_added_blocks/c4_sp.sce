...
if (blk_name.entries(bl) =='c4_sp') then
    j=1:scs_m.objs(blk_objs(bl)).model.ipar(1)
    freq_c=scs_m.objs(blk_objs(bl)).model.rpar'
    tau1=1 ./(2*%pi *freq_c(1:j))
    tau2=1 ./(2*%pi*freq_c(3*j+1:3*j+j))
    c2 = 160e-15 * ones(scs_m.objs(blk_objs(bl)).model.ipar(1),1)
    c1 = 64e-15 * scs_m.objs(blk_objs(bl)).model.rpar(j*7)
    cw = 160e-15 * ones(scs_m.objs(blk_objs(bl)).model.ipar(1),1)
    kap = 0.75
    Ut = 0.258
    ...
    for ss=1:scs_m.objs(bl).model.ipar(1)
        ...
        c4sp_str= '.subckt c4_blk in[0]=net' + string(blk(blk_objs(bl),2))+"_1 in[1]=net"+string(blk(blk_objs(bl),3))+"_1 out[0]=net"+ string(blk(blk_objs(bl),2+numofip))+"_" + string(ss)+" #c4_ota_bias[0] =" +string(sprintf('%1.3e',Ibias1(ss)))+"&c4_ota_bias[1] =" +string(sprintf('%1.3e',Ibias2(ss)));
