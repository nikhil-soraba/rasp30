...
if (blk_name.entries(bl) =='integrator_nmirror') then
    ...
    for ss=1:ii
        ...
        if scs_m.objs(bl).model.rpar(4+ii) == 1 then
            plcvpr = %t;
            plcloc=[plcloc;"net"+string(blk(blk_objs(bl),2+numofip))+"_"+string(ss),string(scs_m.objs(bl).model.rpar(5+ii+(ss-1)*2))+' '+string(scs_m.objs(bl).model.rpar(6+ii+(ss-1)*2))+' 0'];
        end
    end
