function [x,y,typ]=Algo_ADC(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'define' then
        ...
        model.sim=list('Algo_ADC_c',5)
        model.in=[-1;-1;-1;-1;-1] //first/row dimension
        model.in2=[-1;-1;-1;-1;-1] //second/column dimension
        model.intyp=[-1;-1;-1;-1;-1] 
        model.out=[-1]
        model.out2=[-1]
        model.outtyp=[-1]
        model.opar=list(in_out_num) //always a list
