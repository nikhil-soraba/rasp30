function [x,y,typ]=vol_div_fgota(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        model=arg1.model;
        while %t do
            [ok,in_out_num,fgotabias,pbias,mbias,ref_target,exprs]=getvalue('Set voltage divider fgota Parameters',..
            ['Number of voltage divider fgota blocks';'fgota bias'; 'fgota p bias'; 'fgota n bias'; 'reference target current'],list('vec',1,'vec',-1,'vec',-1,'vec',-1,'vec',-1),exprs)
