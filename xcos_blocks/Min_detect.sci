function [x,y,typ]=Min_detect(job,arg1,arg2)
    ...
    case 'set' then
        ...
        while %t do
            [ok, in_out_num, Min_detect_fgswc_ibias, Min_detect_ota0_ibias, exprs]=scicos_getvalue('New Block Parameter',['number of blocks'; 'Min_detect_fgswc_ibias'; 'Min_detect_ota0_ibias'],list('vec',-1, 'vec', -1, 'vec', -1),exprs)
            
            if ~ok then break,end
            if ok then
                model.ipar=in_out_num
                model.rpar=[Min_detect_fgswc_ibias Min_detect_ota0_ibias ]
    ...
    case 'define' then
        ...
        exprs=[sci2exp(in_out_num);sci2exp(Min_detect_fgswc_ibias);sci2exp(Min_detect_ota0_ibias)]
