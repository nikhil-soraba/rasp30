function [x,y,typ]=ladder_filter(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok, in_out_num,forward_ota_current,backward_ota_current, exprs]=scicos_getvalue('New Block Parameter',['number of blocks','forward_ota_current','backward_ota_current'],list('vec',-1,'vec',-1,'vec',-1),exprs)
            ...
            if ok then
                ...
                model.rpar = [forward_ota_current;backward_ota_current]
    ...
    case 'define' then
        in_out_num=1
				forward_ota_current=10e-9
				backward_ota_current=10e-9
        model=scicos_model()
        ...
        model.sim=list('ladder_filter_c',5)
        ...
        exprs=[sci2exp(in_out_num);sci2exp(forward_ota_current);sci2exp(backward_ota_current)] // exprs MUST BE semicolon separated & sci2exp() also converts an expression to string
