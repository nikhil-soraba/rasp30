function [x,y,typ]=TIA_ramp(job,arg1,arg2)
    // Copyright INRIA
    x=[];y=[];typ=[];
    select job
    case 'plot' then
    ...
    case 'set' then
        ...
        while %t do
            [ok,exprs]=scicos_getvalue('New Block Parameter',exprs)
    ...
    case 'define' then
        ...
        exprs=" " // exprs MUST BE semicolon separated & sci2exp() also converts an expression to string
        gr_i=['txt='' TIA_ramp'';';'xstringb(orig(1),orig(2),txt,sz(1),sz(2),''fill'')']
